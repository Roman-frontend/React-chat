...
const Message_jsx_1 = __importDefault(require("./Message/Message.jsx"));
...
exports.Messages = (0, react_1.memo)((props) => {
    const { openPopup, setOpenPopup, dataForBadgeInformNewMsg, setChatsHasNewMsgs, } = props;
    ...
    const userDmIds = (0, client_1.useReactiveVar)(reactiveVars_1.reactiveDirectMessages);
    ...
    const { loading, data: messages, client, } = (0, client_1.useQuery)(queryes_1.GET_MESSAGES, {
    ...
    soket_1.wsSingleton.clientPromise
        ...
        wsClient.onmessage = (response) => {
            ...
            if (parsedRes && parsedRes.text && parsedRes.chatId === chatId) {
                ...
                const chatMessages = oldMsg && oldMsg.messages && oldMsg.messages.chatMessages
                    ? oldMsg.messages.chatMessages
                    : [];
                client.writeQuery({
                    query: queryes_1.GET_MESSAGES,
                    data: {
                        messages: Object.assign(Object.assign({}, oldMsg.messages), { chatMessages: [...chatMessages, parsedRes] }),
                    },
                });
            }
            if (parsedRes && parsedRes.text && parsedRes.chatId !== chatId) {
                ...
                if (dataForBadgeInformNewMsg[0]) {
                    isFirstNewMsgInChat = dataForBadgeInformNewMsg.find((chat) => chat.id === parsedRes.chatId);
                }
                ...
                const filteredChats = dataForBadgeInformNewMsg.filter((chat) => chat.id !== parsedRes.chatId);
                setChatsHasNewMsgs((prev) => [...filteredChats, newChatHasNewMsgs]);
            }
            if ((parsedRes === null || parsedRes === void 0 ? void 0 : parsedRes.message) && parsedRes.message === 'added dm') {
                const storage = JSON.parse(sessionStorage.getItem('storageData'));
                const toStorage = JSON.stringify(Object.assign(Object.assign({}, storage), { directMessages: [...storage.directMessages, parsedRes.id] }));
                sessionStorage.setItem('storageData', toStorage);
                (0, reactiveVars_1.reactiveDirectMessages)([...userDmIds, parsedRes.id]);
                refetch();
            }
            if ((parsedRes === null || parsedRes === void 0 ? void 0 : parsedRes.message) && parsedRes.message === 'removed dm') {
                const storage = JSON.parse(sessionStorage.getItem('storageData'));
                const newDrMsgIds = storage.directMessages.filter((dmId) => dmId !== parsedRes.id);
    ...
    soket_1.wsSingleton.onclose = (response) => {
        const disconnectStatus = response.wasClean
            ? 'DISCONNECTED CLEAN'
            : 'DISCONNECTED BROKEN';
        console.log(`${disconnectStatus} with code ${response.code} reason ${response.reason}`);
    };
    ...
    const renderMessages = (0, react_1.useCallback)(() => {
        if (messages &&
            ...
            return reversedMessages.map((message) => {
                return (<react_1.Profiler id='Message' key={message.id} onRender={callback}>
            <Message_jsx_1.default key={message.id} message={message} openPopup={openPopup} setOpenPopup={setOpenPopup} {...props}/>
          </react_1.Profiler>);
            });
        }
        return null;
